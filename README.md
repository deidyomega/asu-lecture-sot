...
| Not all conflicts receive equal attention (CNN effect) | We worked every case that came through the door - but who came through the door was shaped by State Department admissions priorities, which carry their own biases |
...
> Speaker notes: So let me tie this back to the paper. Each row in this table is a concern Edler and colleagues raised, matched against how our team addressed it operationally. They worry about circular validation - we had three independent data streams. They worry about subjective visual interpretation - our analysts and clinicians never saw each other's data before the overlay. They acknowledge that certain violations - they use gender-based violence as their example - remain invisible to remote sensing. Torture falls into that same category. You can't see it from space. But GIS and temporal data can build the evidentiary scaffolding around the testimony even when the violation itself leaves no satellite-visible trace. They flag the CNN effect, where media attention drives which conflicts get studied. I'd love to say we were immune to that - we worked every case that came through the door. But I have to be honest: who came through the door wasn't random. My caseload was primarily Iraqi refugees. The US State Department decides admissions numbers by region, and those decisions are shaped by foreign policy priorities, which are shaped by media attention. So the CNN effect didn't hit us at the satellite level - it hit us at the intake level. We were downstream of it, not outside it. And they call for more interdisciplinary collaborfation - our team was that collaboration. Clinicians, GIS analysts, investigators, one shared pipeline. The model the paper says the field needs existed. It worked. 
> 
...
> 
> Somehow in the most human of experiences.. we forgot to include the human who experienced it. Thank you.
